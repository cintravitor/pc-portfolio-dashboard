...
let currentTab = 'portfolio-overview'; // Track current tab
...
function switchTab(tabName) {
    ...
    // Load analysis data if switching to analysis tab
    ...
    try {
        ...
        analysisContent.innerHTML = `
            <div class="analysis-section">
                ...
                <p style="color: #ef4444; margin-bottom: 1rem;">${escapeHtml(error.message)}</p>
                <button class="refresh-btn" onclick="switchTab('portfolio-overview')">Go to Portfolio Overview</button>
            </div>
        `;
    } finally {
...
function analyzePortfolioData(data) {
    // Count solutions per stage
    const stageCount = {};
    data.forEach(product => {
        const stage = product.maturity || 'Not specified';
        stageCount[stage] = (stageCount[stage] || 0) + 1;
    });
    
    // Count solutions with/without key metrics
    let withUXMetric = 0;
    let withBIMetric = 0;
    let withBothMetrics = 0;
    let withoutMetrics = 0;
    
    data.forEach(product => {
        const hasUX = product.keyMetricUX && product.keyMetricUX.trim() !== '';
        const hasBI = product.keyMetricBI && product.keyMetricBI.trim() !== '';
        
        if (hasUX) withUXMetric++;
        if (hasBI) withBIMetric++;
        if (hasUX && hasBI) withBothMetrics++;
        if (!hasUX && !hasBI) withoutMetrics++;
    });
    
    // Count by area
    const areaCount = {};
    data.forEach(product => {
        const area = product.area || 'Not specified';
    ...
    let nonRegulatory = 0;
    data.forEach(product => {
        if (product.regulatory && product.regulatory.toLowerCase().includes('yes')) {
            regulatory++;
        } else {
            nonRegulatory++;
        }
    ...
    return {
        ...
        metrics: {
            ...
            withBothMetrics,
            withoutMetrics,
            withAnyMetric: withUXMetric + withBIMetric - withBothMetrics
        },
        areaCount,
        ownerCount,
        regulatory,
...
function displayAnalysis(analysis) {
    ...
    // Sort stage counts
    const sortedStages = Object.entries(analysis.stageCount)
        .sort((a, b) => b[1] - a[1]);
    
    // Sort area counts
    const sortedAreas = Object.entries(analysis.areaCount)
        .sort((a, b) => b[1] - a[1]);
    
    // Sort owner counts
    const sortedOwners = Object.entries(analysis.ownerCount)
        .sort((a, b) => b[1] - a[1]);
    ...
    analysisContent.innerHTML = `
        ...
        <div class="analysis-section">
            ...
            <div class="analysis-highlight">
                <p><strong>Key Insight:</strong> ${getStageSummary(sortedStages, analysis.totalSolutions)}</p>
            </div>
        </div>
        ...
        <div class="analysis-section">
            ...
            <div class="analysis-highlight">
                <p><strong>Coverage:</strong> ${getMetricsSummary(analysis.metrics, analysis.totalSolutions)}</p>
            </div>
        </div>
        ...
        <div class="analysis-section">
            ...
            <div class="analysis-highlight">
                <p><strong>Regulatory Mix:</strong> ${getRegulatoryMix(analysis.regulatory, analysis.totalSolutions)}</p>
            </div>
        </div>
...
/**
 * Helper function: Get stage summary
 */
function getStageSummary(sortedStages, total) {
    if (sortedStages.length === 0) return 'No stage data available.';
    const [topStage, topCount] = sortedStages[0];
    const percentage = Math.round((topCount / total) * 100);
    return `The majority of solutions (${percentage}%) are in the "${topStage}" stage.`;
}

/**
 * Helper function: Get metrics summary
 */
function getMetricsSummary(metrics, total) {
    const anyMetricPercentage = Math.round((metrics.withAnyMetric / total) * 100);
    const bothMetricsPercentage = Math.round((metrics.withBothMetrics / total) * 100);
    return `${anyMetricPercentage}% of solutions have at least one metric defined, with ${bothMetricsPercentage}% having both UX and BI metrics.`;
}

/**
 * Helper function: Get regulatory mix
 */
function getRegulatoryMix(regulatory, total) {
    const percentage = Math.round((regulatory / total) * 100);
    return `${percentage}% of solutions are driven by regulatory demands.`;
}
